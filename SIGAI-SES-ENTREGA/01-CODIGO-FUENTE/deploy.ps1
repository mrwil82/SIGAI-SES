...
if ($healthy) {
    ...
    Write-Host "`n⚠️  Los contenedores pueden estar iniciando aún. Ejecute: docker compose logs -f backend"
}
...
🔧 COMANDOS ÚTILES:
   ...
   Reiniciar API:    docker compose restart backend
   Backup manual:    docker compose exec backend python -m scripts.backup_db
   Detener todo:     docker compose down
   Detener + borrar: docker compose down -v  (¡BORRA LA BASE DE DATOS!)
