...
import { Acta, ActaDetalle } from "./types";
...
const ActaViewModal: React.FC<ActaViewModalProps> = ({
  isOpen,
  onClose,
  acta,
  projectName,
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[680px] mx-4 bg-bg1 p-4 sm:p-6 rounded-xl shadow-neo"
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => {
                if (acta?.id_acta) onDownload(acta.id_acta);
              }}
            >
              Descargar
            </Button>
          ...
          <div>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="text-[9px] text-content-muted text-left px-3 py-2">
                      #
                    </th>
                    <th className="text-[9px] text-content-muted text-left px-3 py-2">
                      Descripción
                ...
                <tbody>
                  {(acta.detalles || []).map((d: ActaDetalle, i: number) => (
                    <tr key={i} className="border-t border-bg4">
                      <td className="px-3 py-2">{i + 1}</td>
                      <td className="px-3 py-2">
                        {d.descripcion || d.id_item || "—"}
                      </td>
                      <td className="px-3 py-2">{d.cantidad ?? "—"}</td>
                      <td className="px-3 py-2">
                        {d.notas_estado || d.observaciones || "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
