...
class TestActas:
    async def test_create_acta(self, test_db, sample_user_data, sample_item_data):
        ...
        item = await crud_inventory.create_item(
            test_db, item=item_in, current_user_id=cast(int, user.id_usuario)
        )
        assert item is not None

        detalle = DetalleActaCreate(
            id_item=cast(int, item.id_item), cantidad=Decimal("2.0")
        )
        acta_in = ActaEntregaCreate(
            id_usuario_tecnico=cast(int, user.id_usuario),
            id_usuario_representante=cast(int, user.id_usuario),
            tipo_acta="ENTREGA_HERRAMIENTA",
            estado_acta="BORRADOR",
            detalles=[detalle],
        )
        acta = await crud_deliveries.create_acta(
            test_db, acta_in=acta_in, current_user_id=cast(int, user.id_usuario)
        )
    ...
    async def test_get_actas(self, test_db, sample_user_data, sample_item_data):
        ...
        detalle = DetalleActaCreate(
            id_item=cast(int, item.id_item), cantidad=Decimal("1.0")
        )
        acta_in = ActaEntregaCreate(
            id_usuario_tecnico=cast(int, user.id_usuario),
            id_usuario_representante=cast(int, user.id_usuario),
        ...
        await crud_deliveries.create_acta(
            test_db, acta_in=acta_in, current_user_id=cast(int, user.id_usuario)
        )
