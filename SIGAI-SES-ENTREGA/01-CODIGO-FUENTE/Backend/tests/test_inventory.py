...
from app.schemas.inventory import ItemCreate, ItemUpdate, ActivoCreate
...
class TestItemCRUD:
    async def test_create_item(self, test_db, sample_item_data):
        ...
        assert cast(str, item.nombre_equipo) == "Camara Test"
    ...
    async def test_get_item_by_id(self, test_db, sample_item_data):
        item_in = ItemCreate(**sample_item_data)
        created = await crud_inventory.create_item(
            test_db, item=item_in, current_user_id=1
        )
        assert created is not None

        found = await crud_inventory.get_item_by_id(test_db, item_id=cast(int, created.id_item))
        assert found is not None
        assert cast(int, found.id_item) == cast(int, created.id_item)
    ...
    async def test_update_item(self, test_db, sample_item_data):
        ...
        updated = await crud_inventory.update_item(
            test_db, item_id=cast(int, created.id_item), item_in=update_in, current_user_id=1
        )
        assert updated is not None
        assert cast(str, updated.nombre_equipo) == "Camara Updated"
    ...
    async def test_delete_item_soft(self, test_db, sample_item_data):
        ...
        deleted = await crud_inventory.delete_item(
            test_db, item_id=cast(int, created.id_item), current_user_id=1
        )
    ...
    async def test_create_activo(self, test_db, sample_item_data):
        ...
        activo_in = ActivoCreate(id_item=cast(int, item.id_item), serial="SN-TEST-001")
        activo = await crud_inventory.create_activo(
            test_db, activo=activo_in, current_user_id=1
        )
        assert activo is not None
        assert activo.id_activo is not None
        assert cast(str, activo.serial) == "SN-TEST-001"
        assert cast(str, activo.estado_actual) == "DISPONIBLE"
