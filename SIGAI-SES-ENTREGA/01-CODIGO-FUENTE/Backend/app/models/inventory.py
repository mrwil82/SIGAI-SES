...
class Item(Base):
    ...
    stock_bulk = relationship(
        "StockBulk", back_populates="item", uselist=False, cascade="all, delete-orphan"
    )
...
class Activo(Base):
    ...
    id_cliente_actual = Column(
        Integer, ForeignKey("clientes.id_cliente"), nullable=True
    )
    ...
    calificacion_tecnica = Column(
        Enum("BUENO", "RECUPERABLE", "DESECHO", name="calificacion_tecnica"),
        default="BUENO",
    )
...
class EPPAssignacion(Base):
    ...
    estado = Column(
        Enum("ACTIVO", "DEVUELTO", "VENCIDO", "PERDIDO", name="epp_estado"),
