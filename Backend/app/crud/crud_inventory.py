...
async def create_item(db: AsyncSession, item: ItemCreate, current_user_id: int):
    ...
    logger.info(
        "Creando/actualizando item",
        extra={"item_nombre": item.nombre_equipo, "user_id": current_user_id},
    )
    ...
    if ref or cod:
        ...
        result = await db.execute(select(Item).filter(or_(*conditions)))
    ...
    if db_item:
        update_data = item.model_dump(
            exclude_unset=True, exclude={"cantidad_inicial", "ubicacion"}
        )
        update_data["referencia"] = _none_if_empty(update_data.get("referencia"))
        update_data["codigo_item_interno"] = _none_if_empty(
            update_data.get("codigo_item_interno")
        )
        ...
        if cantidad_inicial > 0:
            ...
            if db_stock:
                stock_any = cast(Any, db_stock)
                stock_any.cantidad_actual = (
                    float(stock_any.cantidad_actual or 0) + cantidad_inicial
                )
            else:
                db_stock = StockBulk(
                    id_item=db_item.id_item, cantidad_actual=cantidad_inicial
                )
                db.add(db_stock)
            await db.flush()
            ...
            movimiento = MovimientoInventario(
                ...
                notes=f"Ingreso adicional al actualizar item: {item.nombre_equipo}"
                + (f" | Ubicacion: {item.ubicacion}" if item.ubicacion else ""),
            )
            db.add(movimiento)
    else:
        is_new = True
        ...
        if cantidad_inicial > 0:
            movimiento = MovimientoInventario(
                ...
                origen="COMPRA / REGISTRO MANUAL",
                destino=item.ubicacion or "BODEGA",
                notes=f"Ingreso inicial al crear el item: {item.nombre_equipo}"
                + (f" | Ubicacion: {item.ubicacion}" if item.ubicacion else ""),
            )
            db.add(movimiento)
    ...
    await db.commit()
    logger.info(
        f"Item {'creado' if is_new else 'actualizado'} exitosamente",
        extra={
            "item_id": getattr(db_item, "id_item", None),
...
async def update_item(
    db: AsyncSession, item_id: int, item_in: ItemUpdate, current_user_id: int
):
    logger.info(
        "Actualizando item", extra={"item_id": item_id, "user_id": current_user_id}
    )
    ...
    logger.info(
        ...
        extra={"item_id": item_id, "campos": list(update_data.keys())},
    )
    await create_audit_log(
        db,
        current_user_id,
...
async def delete_item(db: AsyncSession, item_id: int, current_user_id: int):
    logger.info(
        "Eliminando item (soft delete)",
        extra={"item_id": item_id, "user_id": current_user_id},
    )
    result = await db.execute(select(Item).filter(Item.id_item == item_id))
    db_item = result.scalars().first()
    if db_item:
        ...
        await create_audit_log(
            ...
            "DELETE",
            item_id,
            nuevo={"deleted_at": str(getattr(db_item, "deleted_at"))},
        )
    else:
...
async def create_activo(db: AsyncSession, activo: ActivoCreate, current_user_id: int):
    ...
    logger.info(
        ...
        extra={
            "id_activo": getattr(db_activo, "id_activo", None),
            "serial": activo.serial,
        },
    )
    await create_audit_log(
        ...
        "activos",
        "CREATE",
        getattr(db_activo, "id_activo"),
        nuevo=activo.model_dump(),
    )

    result = await db.execute(
        select(Activo)
        .options(joinedload(Activo.item).joinedload(Item.stock_bulk))
        .filter(Activo.id_activo == getattr(db_activo, "id_activo"))
    )
    return result.scalars().first()


async def update_activo(
    db: AsyncSession, activo_id: int, activo_in: ActivoUpdate, current_user_id: int
):
    logger.info(
        "Actualizando activo",
        extra={"id_activo": activo_id, "user_id": current_user_id},
    )
    ...
    if "ubicacion_fisica" in update_data and update_data["ubicacion_fisica"] != getattr(
        db_activo, "ubicacion_fisica"
    ):
        await db.execute(
...
async def get_activos_by_estado(
    db: AsyncSession, estado: str, skip: int = 0, limit: int = 100
):
...
async def update_activo_triaje(
    db: AsyncSession, id_activo: int, triaje_data: dict, current_user_id: int
):
    result = await db.execute(
        select(Activo)
        .options(joinedload(Activo.item).joinedload(Item.stock_bulk))
    ...
    for idx, item_info in enumerate(items_data):
        ...
        if stock:
            setattr(
                stock,
                "cantidad_actual",
                (getattr(stock, "cantidad_actual") or 0) + cantidad,
            )
        else:
        ...
        db.add(movimiento)
        detalles.append(
            {"id_item": item_id, "cantidad": cantidad, "seriales": seriales_creados}
        )
