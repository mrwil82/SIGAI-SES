...
async def create_acta(
    db: AsyncSession, acta_in: ActaEntregaCreate, current_user_id: int
) -> ActaEntrega:
...
async def delete_acta(db: AsyncSession, acta_id: int, current_user_id: int):
    ...
    if db_acta:
        try:
            await db.execute(
                delete(DetalleActaEntrega).where(DetalleActaEntrega.id_acta == acta_id)
            )
            await db.execute(delete(ActaEntrega).where(ActaEntrega.id_acta == acta_id))
            await db.commit()
            await create_audit_log(
                db, current_user_id, "actas_entrega", "DELETE", acta_id
            )
        except Exception as e:
            await db.rollback()
...
async def mark_acta_downloaded(db: AsyncSession, acta_id: int, current_user_id: int):
    ...
    if db_acta:
        ...
        await create_audit_log(
            db, current_user_id, "actas_entrega", "DOWNLOAD", acta_id
        )
    return db_acta


async def update_acta(
    db: AsyncSession, acta_id: int, acta_in: ActaEntregaCreate, current_user_id: int
):
    result = await db.execute(select(ActaEntrega).where(ActaEntrega.id_acta == acta_id))
    db_acta = result.scalars().first()
    if not db_acta:
