...
async def create_user(db: AsyncSession, user: UsuarioCreate, current_user_id: int):
    ...
    if existing:
        raise ValueError(
            f"El correo '{user.email}' ya está registrado por otro usuario."
        )
    ...
    try:
        ...
        await create_audit_log(
            ...
            id_usuario=(
                ...
                else int(getattr(db_user, "id_usuario"))
            ),
            tabla="usuarios",
            accion="CREATE",
...
async def update_user(
    db: AsyncSession, user_id: int, user_in: UsuarioUpdate, current_user_id: int
):
    ...
    if "email" in update_data and update_data["email"] != db_user.email:
        ...
        if existing:
            raise ValueError(
                f"El correo '{update_data['email']}' ya está registrado por otro usuario."
            )
