...
async def get_clientes(db: AsyncSession, skip: int = 0, limit: int = 100):
    total_result = await db.execute(
        select(func.count()).select_from(Cliente).filter(Cliente.deleted_at == None)
    )
...
async def create_cliente(
    db: AsyncSession, cliente: ClienteCreate, current_user_id: int
):
    try:
...
async def update_cliente(
    db: AsyncSession, id_cliente: int, cliente_in: ClienteUpdate, current_user_id: int
):
    ...
    old_data = {
        c.name: getattr(db_cliente, c.name) for c in db_cliente.__table__.columns
    }
    ...
    if db_cliente:
        try:
            ...
            await create_audit_log(
                ...
                "DELETE",
                id_cliente,
                nuevo={"deleted_at": str(getattr(db_cliente, "deleted_at"))},
            )
        except Exception as e:
            await db.rollback()
...
async def get_proveedores(db: AsyncSession, skip: int = 0, limit: int = 100):
    total_result = await db.execute(
        select(func.count()).select_from(Proveedor).filter(Proveedor.deleted_at == None)
    )
    total = total_result.scalar() or 0
    result = await db.execute(
...
async def create_proveedor(
    db: AsyncSession, proveedor: ProveedorCreate, current_user_id: int
):
    try:
        ...
        await create_audit_log(
            ...
            "proveedores",
            "CREATE",
            getattr(db_proveedor, "id_proveedor"),
            nuevo=proveedor.model_dump(),
        )
        return db_proveedor
    except Exception as e:
        await db.rollback()
...
async def update_proveedor(
    ...
    result = await db.execute(
        select(Proveedor).filter(Proveedor.id_proveedor == id_proveedor)
    )
    ...
    old_data = {
        c.name: getattr(db_proveedor, c.name) for c in db_proveedor.__table__.columns
    }
    ...
    if db_proveedor:
        try:
            ...
            await create_audit_log(
                ...
                "DELETE",
                id_proveedor,
                nuevo={"deleted_at": str(getattr(db_proveedor, "deleted_at"))},
            )
        except Exception as e:
            await db.rollback()
...
async def create_proyecto(
    db: AsyncSession, proyecto: ProyectoCreate, current_user_id: int
):
    try:
...
async def update_proyecto(
    ...
    result = await db.execute(
        select(Proyecto).filter(Proyecto.id_proyecto == id_proyecto)
    )
    ...
    old_data = {
        c.name: getattr(db_proyecto, c.name) for c in db_proyecto.__table__.columns
    }
...
async def get_movimientos(db: AsyncSession, skip: int = 0, limit: int = 100):
    ...
    result = await db.execute(
        ...
        .options(
            joinedload(MovimientoInventario.item).joinedload(Item.stock_bulk),
            joinedload(MovimientoInventario.activo),
        )
...
async def create_movimiento(
    db: AsyncSession, mov: MovimientoCreate, current_user_id: int
):
    try:
        ...
        await create_audit_log(
            ...
            "CREATE",
            getattr(db_mov, "id_movimiento"),
            nuevo=mov.model_dump(),
        )
        return db_mov
    except Exception as e:
        await db.rollback()
...
async def update_garantia(
    db: AsyncSession, id_garantia: int, obj_in: GarantiaUpdate, current_user_id: int
):
    result = await db.execute(
        select(Garantia).filter(Garantia.id_garantia == id_garantia)
    )
