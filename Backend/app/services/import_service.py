...
class ImportService:
    ...
    async def _upsert_item_and_stock(
        ...
        generated_code = (
            cod or _slug(nombre) or f"ITEM_{int(datetime.utcnow().timestamp())}"
        )
    ...
    async def process_excel(self, file: UploadFile, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail=f"No se pudo leer el Excel: {e}"
            )

        # Detectar tipo para validación
        tipo = ""
        if "inventario_laboratorio" in filename:
            tipo = "inventario_laboratorio"
        elif "formato_inventario" in filename or "clientes_corporativos" in filename:
            tipo = "formato_inventario_clientes"
        elif "asignacion" in filename or "garantia" in filename:
            tipo = "asignacion_numero"

        errors = self.validate_structure(xl, tipo)
        if errors:
            raise HTTPException(
                status_code=400,
                detail={"mensaje": "Estructura de Excel inválida", "errores": errors},
            )
    ...
    async def _procesar_inventario_laboratorio(
        self, xl: pd.ExcelFile, db: AsyncSession, id_proyecto: Optional[int] = None
    ):
        items_creados = 0
        activos_creados = 0
        ...
        for sheet_name in xl.sheet_names:
            ...
            for _, row in df.iterrows():
                ...
                cantidad = self._to_float(
                    row.get(cantidad_col, 0) if cantidad_col else 0
                )
                ubicacion = (
                    self._clean(row.get(ubicacion_col)) if ubicacion_col else None
                )
                codigo = self._clean(row.get(codigo_col)) if codigo_col else None
                activo_fijo = (
                    self._clean(row.get(activo_fijo_col)) if activo_fijo_col else None
                )
        ...
        for sheet_name in xl.sheet_names:
            ...
            for _, row in df.iterrows():
                ...
                await self._upsert_item_and_stock(
                    db=db,
                    nombre=nombre,
                    referencia=self._clean(row.get("REFERENCIA")),
                    codigo=self._clean(row.get("CÓDIGO")),
                    marca=self._clean(row.get("MARCA")),
                    categoria="MONITOREO",
                    sub_categoria=sheet_name,
    ...
    def validate_structure(self, xl: pd.ExcelFile, tipo: str) -> List[str]:
        ...
        if tipo == "inventario_laboratorio":
            if not any(
                "INVENTARIO" in str(s).upper() or "STOCK" in str(s).upper()
