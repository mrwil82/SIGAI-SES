...
const EditActaModal: React.FC<EditActaModalProps> = ({
  isOpen,
  onClose,
  acta,
  onSaved,
}) => {
  const [localActa, setLocalActa] = useState<Acta | null>(acta || null);
  ...
  if (!isOpen || !acta || !localActa) return null;
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[500px] mx-4 bg-bg1 p-4 sm:p-6 rounded-xl shadow-neo"
      ...
      >
        ...
        {error && (
          <div className="mb-4 p-3 rounded-lg bg-danger/10 border border-danger/20 text-danger text-xs">
            {error}
          </div>
        )}
        ...
        <FormGroup label="Número de Acta" htmlFor="numero_acta">
          <NeoInput
            id="numero_acta"
            value={localActa.numero_acta || ""}
            onChange={(e) =>
              setLocalActa({ ...localActa, numero_acta: e.target.value })
            }
          />
        </FormGroup>
        <FormGroup label="Tipo de Acta" htmlFor="tipo_acta">
          <NeoSelect
            id="tipo_acta"
            value={localActa.tipo_acta || ""}
            onChange={(e) =>
              setLocalActa({ ...localActa, tipo_acta: e.target.value })
            }
          >
            {ACTA_TYPES.map((t) => (
        ...
        <FormGroup label="Estado" htmlFor="estado_acta">
          <NeoSelect
            id="estado_acta"
            value={localActa.estado_acta || ""}
            onChange={(e) =>
              setLocalActa({ ...localActa, estado_acta: e.target.value })
            }
          >
            {ACTA_ESTADOS.map((e) => (
        ...
        <FormGroup label="Observaciones" htmlFor="observaciones">
          <NeoTextarea
            id="observaciones"
            value={localActa.observaciones || ""}
            onChange={(e) =>
              setLocalActa({ ...localActa, observaciones: e.target.value })
            }
          />
        </FormGroup>
