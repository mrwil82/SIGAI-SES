...
const Login: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      setError(Array.isArray(detail) ? detail.map((e: any) => e.msg).filter(Boolean).join('; ') : (detail || 'Error al iniciar sesión. Verifique sus credenciales.'));
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="flex flex-col items-center mb-10 relative">
          ...
          <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-emerald-primary/20 to-emerald-primary/5 flex items-center justify-center shadow-[0_0_40px_rgba(0,194,106,0.2)] mb-6 border border-emerald-primary/30 backdrop-blur-md">
            <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 32 32" className="drop-shadow-[0_0_10px_rgba(0,194,106,0.5)]">
              <rect x="2" y="2" width="28" height="28" rx="7" fill="none" stroke="url(#loginAccent)" stroke-width="1.5"/>
              <g transform="translate(8,8)">
                <rect x="1" y="3" width="14" height="11" rx="1.5" fill="none" stroke="url(#loginAccent)" stroke-width="1.5"/>
                <rect x="1" y="1" width="14" height="3" rx="1" fill="url(#loginAccent)" opacity="0.9"/>
          ...
          <h1 className="text-3xl font-black tracking-tighter text-white">SIGAI-<span className="text-emerald-primary">SES</span></h1>
          <p className="text-content-muted text-[10px] uppercase tracking-[0.4em] mt-3 font-mono text-center">Securitas Lab Control</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-widest text-[#9AB8A4] ml-1 font-bold">Credencial de Acceso</label>
              <div className="relative group">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5A7A65]" size={18} />
                <input
                  {...register('username', { required: 'El correo/usuario es requerido' })}
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-widest text-[#9AB8A4] ml-1 font-bold">Contraseña de Seguridad</label>
              <div className="relative group">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5A7A65]" size={18} />
                <input
        ...
        <div className="mt-10 text-center">
          <p className="text-[8px] text-white/30 mt-4 uppercase tracking-widest">© 2024 Lab Digital Securitas - v1.0.0</p>
        </div>
      </div>
    </div>
  );
