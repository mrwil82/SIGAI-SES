...
export const createInventoryItem = async (data: object) => {
...
export const getActivos = async (page: number = 1, page_size: number = 500) => {
  const response = await api.get(
    `/inventory/activos?page=${page}&page_size=${page_size}`,
  );
  return response.data;
};

export const createActivo = async (data: object) => {
  return await api.post("/inventory/activos", data);
};

export const updateActivo = async (id: number, data: object) => {
...
export const crearDesmonteBulk = async (data: {
...
export const downloadTemplate = async (module: string) => {
  ...
  await downloadFromFetch(
    `${baseUrl}/import/templates/${module}`,
    `Plantilla_${module}.xlsx`,
  );
};
