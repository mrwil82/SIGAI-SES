  ...
  const filtered = search
    ? options.filter(
        (o) =>
          ...
          (o.searchTerms &&
            o.searchTerms.toLowerCase().includes(search.toLowerCase())),
      )
    : options;
      ...
      {selected && !isOpen ? (
        ...
        <div className="flex items-center gap-2 w-full h-10 px-3 rounded-xl border border-bg3 bg-bg2 focus-within:border-emerald-500/30 transition-colors min-w-0">
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="p-0.5 rounded hover:bg-emerald-primary/5 text-content-muted transition-colors"
            >
              <X size={14} />
            </button>
          )}
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((opt) => (
              ...
              >
                ...
                <span
                  className={opt.value === value ? "flex-1" : "flex-1 ml-6"}
                >
                  {opt.label}
                </span>
              </div>
