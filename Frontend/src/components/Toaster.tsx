...
> = {
  success: {
    icon: CheckCircle2,
    iconBg: "bg-emerald-primary/10",
    iconColor: "text-emerald-primary",
    titleColor: "text-emerald-primary",
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 p-4 pb-3">
        <div
          className={`w-7 h-7 rounded-[8px] flex items-center justify-center flex-shrink-0 mt-0.5 ${cfg.iconBg}`}
        >
          <Icon size={14} className={cfg.iconColor} strokeWidth={2.5} />
        </div>
        <div className="flex-1 min-w-0">
          <p
            className={`text-[10px] font-bold uppercase tracking-[0.1em] leading-snug ${cfg.titleColor}`}
          >
            {toast.title}
          </p>
          {toast.description && (
