...
> [!NOTE]
> Este manual esta dirigido al **personal de TI y Administradores** encargados de la gestion avanzada del sistema SIGAI-SES en SES — Seguridad Electrónica. Como administrador, tiene **acceso total** al sistema incluyendo configuracion global, gestion de usuarios, auditoria y todos los modulos operativos.
...
| **Codigo Empleado** | Codigo interno de SES | Si |
...
| `ADMIN_EMAIL` | Email del admin inicial | `admin@ses-electronica.com` |
| `ADMIN_PASSWORD` | Password del admin inicial | *(configurar en .env)* |
...
| **Logo no aparece en PDF** | Ruta de imagen incorrecta | Verifique que `SES-logo-light.png` existe en `Backend/app/static/` |
